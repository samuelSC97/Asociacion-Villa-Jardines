const AdminPagos = (() => {
  let _anio = new Date().getFullYear(), _seccion = 'cuotas', _pagoId = null;
  ...
  async function _renderCuotas(el) {
    const [{ data: cuotas }, { data: todos }] = await Promise.all([
      ...
      db.from('vecinos').select('id,nombre,mz,lote,exonerado').eq('activo', true).order('mz').order('lote')
    ]);
    const pagMap  = {};
    ...
    el.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
        <div class="year-nav" style="flex:1;margin-bottom:0">
          <button onclick="AdminPagos.anio(-1)">‹</button>
          <span>Cuotas sociales ${_anio}</span>
          <button onclick="AdminPagos.anio(1)">›</button>
        </div>
        <button class="btn btn-sm btn-outline no-print" style="flex-shrink:0;margin-left:8px" onclick="window.print()">🖨️ PDF</button>
      </div>
      <div class="metrics">
        ...
        <div class="metric"><div class="metric-val c-red">${noExon.filter(v => !pagMap[v.id]).length}</div><div class="metric-lbl">Pendientes</div></div>
      </div>

      <div class="sec-title">Registrar pago cuota social</div>
      <div class="card">
        <div class="field"><label>Vecino</label>
          <select id="cs-vid" onchange="AdminPagos.actualizarDefCuota()">
            ${noExon.map(v => {
              const pag  = pagMap[v.id] || 0;
              const debe = Math.max(0, 24 - pag);
              return `<option value="${v.id}" data-debe="${debe}">${esc(v.nombre.split(',')[0].trim())} — Mz ${esc(v.mz)}-${esc(v.lote)} (${debe > 0 ? 'debe S/' + debe : '✓ al día'})</option>`;
            }).join('')}
          </select>
        </div>
        <div class="grid-2">
          <div class="field"><label>Monto S/</label><input type="number" id="cs-pago-monto" value="${firstDebe > 0 ? firstDebe : 24}" min="1" step="1"></div>
          <div class="field"><label>Fecha</label><input type="date" id="cs-pago-fecha" value="${today()}"></div>
        </div>
        <button class="btn btn-green" onclick="AdminPagos.registrarCuota()">💰 Registrar pago</button>
      </div>

      <div class="sec-title">Estado ${_anio} por vecino</div>
      <div class="card card-flush">
        ${noExon.map(v => {
          const pag = pagMap[v.id] || 0;
          return `<div class="pago-det-row">
            <div><div style="font-weight:500">${esc(v.nombre.split(',')[0].trim())}</div><div style="font-size:11px;color:var(--text2)">Mz ${esc(v.mz)}-${esc(v.lote)}</div></div>
      ...
      <div class="card card-flush">
        ${(cuotas || []).map(c => `<div class="hist-row">
          <div class="hist-left">
            <div class="hist-evento">${esc(c.vecinos?.nombre?.split(',')[0].trim() || '?')}</div>
            <div class="hist-fecha">${formatFecha(c.fecha_pago)}${c.nota ? ' · ' + esc(c.nota) : ''}</div>
          </div>
  ...
  function actualizarDefCuota() {
    const sel   = document.getElementById('cs-vid');
    const opt   = sel?.options[sel.selectedIndex];
    const debe  = parseInt(opt?.dataset?.debe) || 0;
    const montoEl = document.getElementById('cs-pago-monto');
    if (montoEl) montoEl.value = debe > 0 ? debe : 24;
  }

  async function registrarCuota() {
    const vid   = parseInt(document.getElementById('cs-vid').value);
    const monto = parseFloat(document.getElementById('cs-pago-monto').value);
    const fecha = document.getElementById('cs-pago-fecha').value;
    if (!vid || !monto || !fecha) { showToast('Completa todos los campos', 'err'); return; }
    showLoading();

    const { data: hist } = await db.from('cuotas_sociales').select('anio,monto').eq('vecino_id', vid);
    const paidByYear = {};
    (hist || []).forEach(c => { paidByYear[c.anio] = (paidByYear[c.anio] || 0) + parseFloat(c.monto); });

    const anioFecha = parseInt(fecha.slice(0, 4));
    const rows  = [];
    let restante = monto;

    // Apply to oldest deficit years first (up to 3 years back)
    for (let year = anioFecha - 2; year <= anioFecha && restante > 0; year++) {
      const deficit = Math.max(0, 24 - (paidByYear[year] || 0));
      if (deficit > 0) {
        const aplica = Math.min(deficit, restante);
        rows.push({ vecino_id: vid, anio: year, monto: aplica, fecha_pago: fecha });
        restante -= aplica;
      }
    }
    // Advance payment for future years if remaining
    for (let year = anioFecha + 1; year <= anioFecha + 2 && restante > 0; year++) {
      const capacity = Math.max(0, 24 - (paidByYear[year] || 0));
      if (capacity > 0) {
        const aplica = Math.min(capacity, restante);
        rows.push({ vecino_id: vid, anio: year, monto: aplica, fecha_pago: fecha });
        restante -= aplica;
      }
    }
    ...
    if (rows.length) {
      const { error } = await db.from('cuotas_sociales').insert(rows);
      hideLoading();
      if (error) { showToast('Error: ' + error.message, 'err'); return; }
      const desc = rows.map(r => `S/${r.monto} → ${r.anio}`).join(' · ');
      showToast(`✓ ${desc}`);
    } else {
      hideLoading();
      showToast('Vecino no tiene déficit en los años revisados', 'info');
    }
    render();
  }
  ...
  async function _renderOtros(el) {
    ...
    el.innerHTML = `
      ...
      ${(pagos || []).filter(p => p.activo).map(p => `
        <div class="opago-card" onclick="AdminPagos.verPago(${p.id})">
          <div class="opago-header">
            <div><div style="font-weight:600">${esc(p.nombre)}</div><div style="font-size:11px;color:var(--text2)">${formatFecha(p.fecha_creacion)} · S/${p.monto} por vecino${p.descripcion ? ' · ' + esc(p.descripcion) : ''}</div></div>
      ...
      ${(pagos || []).filter(p => !p.activo).map(p => `
        <div class="opago-card" onclick="AdminPagos.verPago(${p.id})">
          <div class="opago-header">
            <div><div style="font-weight:600;color:var(--text2)">${esc(p.nombre)}</div><div style="font-size:11px;color:var(--text3)">${formatFecha(p.fecha_creacion)} · S/${p.monto}</div></div>
  ...
  async function _renderOtroDetalle(el, id) {
    ...
    el.innerHTML = `
      ...
      <div class="card">
        <div class="card-title">${esc(pago.nombre)}</div>
        ${pago.descripcion ? `<div style="font-size:13px;color:var(--text2);margin-bottom:8px">${esc(pago.descripcion)}</div>` : ''}
        <div class="metrics">
          <div class="metric"><div class="metric-val c-green">S/${totalRec}</div><div class="metric-lbl">Recaudado</div></div>
      ...
      <div class="card">
        <div class="field"><label>Vecino</label>
          <select id="op-vecino">
            ${(vecinos || []).filter(v => !pagadosIds.has(v.id)).map(v => `<option value="${v.id}">${esc(v.nombre.split(',')[0].trim())} — Mz ${esc(v.mz)}-${esc(v.lote)}</option>`).join('')}
          </select>
        </div>
        <div class="grid-2">
      ...
      <div class="card card-flush">
        ${(vecinos || []).map(v => { const p = (pagados || []).find(x => x.vecino_id === v.id); return `<div class="pago-det-row">
          <div><div style="font-weight:500">${esc(v.nombre.split(',')[0].trim())}</div><div style="font-size:11px;color:var(--text2)">Mz ${esc(v.mz)}-${esc(v.lote)}${p ? ' · ' + formatFecha(p.fecha_pago) : ''}</div></div>
  ...
  function getSec()     { return _seccion; }
  ...
  async function crearCobro() {
    ...
    if (!nombre || !monto || !fecha) { showToast('Completa todos los campos', 'err'); return; }
    showLoading();
    const { error } = await db.from('otros_pagos').insert({ nombre, descripcion: desc, monto, fecha_creacion: fecha, activo: true });
    hideLoading();
    if (error) showToast('Error: ' + error.message, 'err');
    else { showToast('✓ Cobro creado'); render(); }
  }
  ...
  async function registrarOtroPago(pagoId) {
    ...
    if (!vid || !monto || !fecha) { showToast('Completa todos los campos', 'err'); return; }
    showLoading();
    const { error } = await db.from('otros_pagos_vecinos').insert({ pago_id: pagoId, vecino_id: vid, monto_pagado: monto, fecha_pago: fecha });
    hideLoading();
    if (error) showToast('Error: ' + error.message, 'err');
    else { showToast('✓ Pago registrado'); render(); }
  }
  ...
  return { render, sec, getSec, anio, actualizarDefCuota, registrarCuota, verPago, volverOtros, crearCobro, registrarOtroPago, toggleCobro };
})();
