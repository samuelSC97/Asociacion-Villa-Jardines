...
const AdminDocumentos = (() => {
  ...
  function _draw(el, docs, agenda) {
    const categorias = ['acta', 'otro'];
    const a = agenda || {};

    el.innerHTML = `
      <!-- PRÓXIMA AGENDA -->
      <div class="sec-title">Próxima agenda / convocatoria</div>
      <div class="card">
        <div class="card-title">Configurar agenda visible para vecinos</div>
        <div class="field"><label>Título</label>
          <input type="text" id="ag-titulo" value="${a.titulo || ''}" placeholder="Ej: Asamblea ordinaria junio 2025">
        </div>
        <div class="grid-2">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
          <input type="checkbox" id="ag-activa" ${a.activa ? 'checked' : ''} style="width:auto;margin:0">
          <label for="ag-activa" style="text-transform:none;font-size:13px;font-weight:400;letter-spacing:0">Activa (visible para vecinos)</label>
        </div>
      ...
      <div class="card">
        <div class="field"><label>Título del documento</label>
          <input type="text" id="doc-titulo" placeholder="Ej: Acta asamblea marzo 2025">
        </div>
        <div class="grid-2">
          ...
          <div class="field"><label>Fecha del documento</label>
            <input type="date" id="doc-fecha" value="${today()}">
          </div>
        </div>
        <div class="field"><label>Archivo PDF</label>
          <input type="file" id="doc-file" accept=".pdf" style="padding:8px">
        </div>
        <button class="btn btn-blue" onclick="AdminDocumentos.subirPDF()">📎 Subir documento</button>
      ...
      ${categorias.map(cat => {
        ...
        return `
          <div class="sec-title">${cat === 'acta' ? '📋 Actas de asamblea' : '📄 Otros documentos'}</div>
            ...
            ${catDocs.map(d => `
              <div class="pdf-item">
                ...
                <div style="display:flex;gap:6px">
                  ...
                  <button class="btn btn-sm" style="color:var(--red);border-color:var(--red)" onclick="AdminDocumentos.eliminar(${d.id})">✕</button>
                </div>
              </div>`).join('')}
          </div>`;
      }).join('')}
  ...
  async function guardarAgenda(id) {
    ...
    if (!datos.titulo || !datos.fecha_evento) { alert('Completa el título y la fecha del evento'); return; }
    showLoading();
    if (id) {
  ...
  async function subirPDF() {
    ...
    if (!titulo || !fecha) { msgEl.innerHTML = '<div class="msg msg-err">Completa el título y la fecha</div>'; return; }
    ...
    const { data: upData, error: upErr } = await db.storage.from('documentos').upload(filename, file, { contentType: 'application/pdf' });

    if (upErr) { msgEl.innerHTML = '<div class="msg msg-err">Error al subir: ' + upErr.message + '</div>'; return; }
    const { data: urlData } = db.storage.from('documentos').getPublicUrl(filename);
    const url_pdf = urlData.publicUrl;

    await db.from('documentos').insert({ titulo, categoria: cat, fecha, url_pdf, visible: true });
    msgEl.innerHTML = '<div class="msg msg-ok">✓ Documento subido correctamente</div>';
  ...
  return { render, guardarAgenda, subirPDF, eliminar };
})();
