const AdminVecinos = (() => {
  ...
  async function _renderDetalle(id) {
    ...
    const _guard = (apoyos || []).filter(a => a.estado === 'guardadito').reduce((s, a) => s + parseFloat(a.monto), 0);
    _detalleData = { v, asist: asist || [], deudasAnt: deudasAnt || [], guardadito: _guard };
  ...
  function exportar() {
    ...
    _exportarHtml(_detalleData.v, _detalleData.asist, _detalleData.deudasAnt, _detalleData.guardadito || 0);
  }

  function _exportarHtml(v, asist, deudasAnt, guardadito) {
    const faltas      = asist.filter(a => a.estado === 'F');
    ...
    // Deudas anteriores al final (son las más antiguas)
